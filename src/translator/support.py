...
from ..core.schema import SegmentList
...
class CheckpointManager:
    ...
    def __init__(self, settings: "Settings"):
        ...
        doc_hash = (
            get_file_hash(settings.files.document_path)
            if settings.files.document_path
            else "unknown"
        )

        # 从 settings 中获取项目目录
        base_dir = (
            Path(settings.files.output_base_dir)
            if isinstance(settings.files.output_base_dir, str)
            else settings.files.output_base_dir
        )
    ...
    def _load_checkpoint(self):
        ...
        if self.checkpoint_file.exists():
            try:
                ...
                completed_count = len(
                    self.checkpoint_data.get("completed_segments", [])
                )
    ...
    def get_pending_segments(self, all_segments: SegmentList) -> SegmentList:
        ...
        for seg in all_segments:
            ...
            # 条件2: 翻译结果为空或包含失败标记
            has_failed_content = (
                not seg.translated_text
                or seg.translated_text.startswith("[Failed")
                or seg.translated_text.endswith("Failed]")
            )
        ...
        if pending:
            logger.info(
                f"🔄 检测到 {len(pending)} 个待翻译段落 (共 {len(all_segments)} 个)"
            )
        else:
...
class CachePersistenceManager:
    ...
    def __init__(self, settings: "Settings"):
        ...
        self.project_dir = (
            base_dir if base_dir.name == doc_hash else (base_dir / doc_hash)
        )
    ...
    def _cleanup_expired_caches(self):
        ...
        for cache_type in self.cache_metadata:
            ...
            expired_keys = [
                k
                for k, v in self.cache_metadata[cache_type].items()
    ...
    def register_system_cache(
        self, cache_name: str, content_hash: str, ttl_hours: float = 1.0
    ) -> bool:
    ...
    def get_system_cache(self, content_hash: str) -> Optional[str]:
        ...
        for cache_key, cache_info in self.cache_metadata["system_instruction"].items():
            if cache_info.get(
                "content_hash"
            ) == content_hash and current_time < cache_info.get("expiry_time", 0):
    ...
    def get_context_cache(self, context_hash: str) -> Optional[str]:
        ...
        for cache_key, cache_info in self.cache_metadata["context"].items():
            if cache_info.get(
                "context_hash"
            ) == context_hash and time.time() < cache_info.get("expiry_time", 0):
    ...
    def get_or_create_system_cache(
        ...
        with self._cache_creation_lock:
            ...
            if content_hash in self._pending_cache_creation:
                logger.info("⏳ 检测到其他 worker 正在创建缓存，等待完成...")
                # 等待缓存创建完成（最多等待 30 秒）
                ...
                while content_hash in self._pending_cache_creation:
                    ...
                    if timeout_remaining <= 0:
                        ...
                        break
                    # 释放锁并等待通知
                    self._cache_created_condition.wait(
        ...
        try:
            ...
            with self._cache_creation_lock:
                ...
                logger.debug("🔓 缓存创建流程结束，已通知等待线程")

    def get_or_create_glossary_cache(
        self, glossary: Dict[str, str], model_name: str
    ) -> Optional[str]:
        """
        统一的术语表缓存获取或创建方法
        ...
        try:
            ...
            cache_name = cache.name
            logger.info(
                f"✅ 术语表缓存已创建: {cache_name[:50]}... ({len(glossary)}项)"
            )
...
class PromptManager:
    ...
    def __init__(self, settings: "Settings"):
        ...
        if is_cloud_provider:
            ...
            self.system_instruction_base = self._load_prompt_template(
                "system_instruction.md"
            )
            self.text_translation_prompt = self._load_prompt_template(
        ...
        # 视觉和JSON修复prompt保持不变
        self.vision_translation_prompt = self._load_prompt_template(
    ...
    def _load_prompt_template(self, template_name: str) -> str:
        """从文件加载 Prompt 模板"""
        path = (
            Path(__file__).parent.parent.parent / "config" / "prompts" / template_name
        )
        try:
...
"""
    ...
    def format_text_prompt(
        self, context: str, input_json: str, glossary: str = ""
    ) -> str:
        """
        格式化文本翻译的完整提示（用户消息部分）
    ...
    def format_title_prompt(self, text_list: str) -> str:
        ...
        """
        style = (
            self.mode_entity.style if self.mode_entity else "Professional and accurate"
        )
...
Return ONLY the JSON object."""
    ...
    def format_json_repair_prompt(
        self, original_prompt: str, broken_json: str, error_details: str
    ) -> str:
        """
        格式化 JSON 修复提示
