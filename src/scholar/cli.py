# -*- coding: utf-8 -*-
"""
Scholar Digest CLI 实现
从 scholar_main.py 下沉而来：argparse 构建 + 各子命令的执行逻辑。
scholar_main.py 只留薄入口（import + main() 转发），保持历史命令行调用方式不变。
"""
import sys
import json
import argparse
...
def add_digest_arguments(parser):
    """digest 子命令的参数定义。

    digest 子命令与「无子命令」的旧格式兼容参数完全重复，抽成单函数两处复用
    （main() 里 digest_parser 与 legacy_parser 各调用一次），避免定义漂移。
    """
    # 配置文件
    parser.add_argument('--config', type=str, default='config/scholar.env', help='配置文件路径')

    # 邮件获取参数
    parser.add_argument('--days', type=int, default=None, help='获取最近 N 天的邮件')
    parser.add_argument('--max-emails', type=int, default=None, help='最大处理邮件数量')
    parser.add_argument('--all', action='store_true', help='获取所有历史邮件')
    parser.add_argument('--month', type=str, default=None, help='按月回填 YYYY-MM（Gmail/PubMed/arXiv 都按此月区间检索）')
    parser.add_argument('--since', type=str, default=None, help='起始日期 YYYY-MM-DD（区间回填）')
    parser.add_argument('--until', type=str, default=None, help='结束日期 YYYY-MM-DD（区间回填）')

    # LLM 参数
    parser.add_argument('--provider', type=str, default=None,
                         choices=['gemini', 'deepseek', 'openai-compatible'],
                         help='LLM 提供商（覆盖配置文件）')
    parser.add_argument('--model', type=str, default=None, help='LLM 模型名称（覆盖配置文件）')

    # 处理参数
    parser.add_argument('--batch-size', type=int, default=None, help='LLM 批量处理大小')
    parser.add_argument('--filter-mode', type=str, default=None,
                         choices=['llm', 'keyword'],
                         help='白名单过滤模式：llm=LLM相关性裁决（默认），keyword=关键词匹配')
    parser.add_argument('--no-translate', action='store_true', help='不翻译摘要')
    parser.add_argument('--no-summary', action='store_true',
                         help='翻译已关时也跳过 LLM 批处理步骤（不产出独立总结，仅省关键词/相关度/优先级评分）')
    parser.add_argument('--no-mark-read', action='store_true', help='不标记邮件为已读')
    parser.add_argument('--external', action='store_true', help='启用 PubMed/arXiv 外部检索来源')
    parser.add_argument('--no-external', action='store_true', help='禁用 PubMed/arXiv 外部检索来源')
    ...
    parser.add_argument('--export-csv', action='store_true', help='额外导出 CSV 格式')
...
def run_digest(args, settings):
    ...
    # 名不副实提醒：该开关不产出独立总结内容，只决定是否仍跑 LLM 批处理步骤
    # （关键词/相关度/优先级评分）——见 ProcessingSettings.generate_summary 注释。
    logger.info("  LLM 批处理步骤（无独立总结）: {}".format('是' if settings.processing.generate_summary else '否'))
...
def run_deep_research(args, settings):
    ...
    # 转换为 PaperSegment 对象（反序列化下沉到 schema.PaperSegment.from_digest_json）
    from src.scholar.schema import PaperSegment
    segments = []
    for seg_data in data.get('segments', []):
        try:
            segments.append(PaperSegment.from_digest_json(seg_data))
...
示例用法:
    ...
    python scholar_main.py deep-research --papers output/digest.json --target-words 20000
        ...
        dr_parser.add_argument('--topic', type=str, default=None,
                                help='研究主题；留空时从 config/research_profile.yaml 的研究方向派生')
...
示例用法:
    ...
    # 先用 cron 无人值守出 digest，人在（Zotero 开着）时再执行本命令推送
        ...
        if args.command is None:
            # 为了向后兼容，解析旧格式的参数（与 digest 子命令共用 add_digest_arguments）
            legacy_parser = argparse.ArgumentParser()
            add_digest_arguments(legacy_parser)
