...
class ProcessingSettings(BaseModel):
    ...
    # 依据：263 篇实测正文长度 p75=100,086、p90=128,272。
    #
    # 2026-08-28 订正：原值 120000 配注释「最坏块数 11，正好被 12 的块顶卡住」——
    # 算术是错的。块数 = ceil((max_chars - overlap) / (size - overlap))，按
    # pdf_ingest.chunk_text 的默认 size=12000/overlap=600 代入：
    #   120000 → 11 块      126000 → 11 块      137400 → 12 块
    # 即 max_chunks=12 在 120000 下**恒不生效**，白留一整块从未被使用的额度。
    # 现取 137400 = 恰好吃满 12 块，覆盖率从 p75 抬到 p90 之上，不增加块顶。
    #
    # ⚠️ 这个数与 pdf_ingest._SYNTH_NOTES_BUDGET 是**零和耦合**：汇总步把所有块笔记
    # 塞进一个固定预算，块数一涨，每块配额就摊薄（11 块时每块 5,432 字符，而实测块
    # 笔记约 5.8k，已在裁剪）。只抬本值不抬预算 = 拿已读章节的保真度去换尾部覆盖。
    # 两个数必须一起改，改完跑 test_closeread_chunk_coupling.py。
