# -*- coding: utf-8 -*-
"""
全文精读：OA/arXiv 论文 → 下载 PDF → 抽全文 → 强模型精读 → 结构化 + 句级三色联想。

三色联想关联研究主线（MNAR/MA-GCT）：方法学创新 / 重要发现 / 研究背景（句级、可多）。
...
每个小节由若干句子组成。对**与研究主线相关**的句子打一个标记 tag：
- "方法学创新"：该句描述的方法/建模思路，对我的研究有方法学借鉴价值
- "重要发现"：该句是对我研究有价值的实证发现/结论
- "研究背景"：该句提供我研究相关的背景/动机/现状
其余句子 tag 置为 null。允许同一小节里不同句子有不同 tag（句级、可多）。

## 只输出 JSON（不要额外文字）
{{
  "sections": [
    {{"heading": "研究问题", "sentences": [{{"text": "……", "tag": null}}]}},
    {{"heading": "方法与数据", "sentences": [{{"text": "……", "tag": "方法学创新"}}]}},
    {{"heading": "关键结论", "sentences": [{{"text": "……", "tag": "重要发现"}}]}},
    {{"heading": "可质疑点", "sentences": [{{"text": "……", "tag": null}}]}},
    {{"heading": "对我研究的联想", "sentences": [{{"text": "……", "tag": "研究背景"}}]}}
  ]
}}"""
