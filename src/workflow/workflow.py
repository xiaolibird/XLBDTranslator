...
from ..translator import CheckpointManager, GeminiTranslator, OpenAICompatibleTranslator
...
class TranslationWorkflow:
    ...
    def __init__(self, settings: Settings):
        ...
        self.project_dir = create_output_directory(
            settings.files.output_base_dir, self.project_name
        )
    ...
    def _emergency_save(self) -> None:
        ...
        try:
            if self.all_segments:
                ...
                logger.info(
                    f"   - structure_map.json 已保存 ({len(self.all_segments)} segments)"
                )
            if self.checkpoint:
    ...
    def _optimize_batch_size_for_provider(self):
        ...
        if provider in {"deepseek", "openai", "openai-compatible"}:
            ...
            # 计算安全的batch_size
            safe_batch_size = max(
                1, (max_safe_chars - prompt_overhead) // max_chunk_size
            )
            optimized_batch_size = min(
                original_batch_size, safe_batch_size, 3
            )  # 最多3作为硬上限

            logger.info(f"🔧 参数优化分析 ({provider.upper()}):")
            logger.info(
                f"   📊 当前配置: batch_size={original_batch_size}, max_chunk_size={max_chunk_size}"
            )
            logger.info(
                f"   📏 估算字符量: {estimated_chars_per_batch:,} 字符/批次 (不含prompt)"
            )
            logger.info(
                f"   ⚠️  安全上限: {max_safe_chars:,} 字符/批次 (含{prompt_overhead:,}字符prompt开销)"
            )
            logger.info(
                f"   🎯 优化结果: batch_size {original_batch_size} → {optimized_batch_size}"
            )

            if optimized_batch_size < original_batch_size:
                new_estimated_chars = (
                    optimized_batch_size * max_chunk_size + prompt_overhead
                )
                logger.info(
                    f"   ✅ 新配置字符量: {new_estimated_chars:,} 字符/批次 (安全范围内)"
                )
                logger.info(
                    f"   💡 备选方案: 可考虑减少 max_chunk_size 至 {max_chunk_size//2} 以增加batch_size"
                )
            else:
                logger.info(
                    f"   ✅ 当前配置已安全: {estimated_chars_per_batch + prompt_overhead:,} 字符/批次"
                )
            ...
            if optimized_batch_size < original_batch_size:
                ...
                logger.info(
                    f"   ✅ 参数保持: batch_size = {original_batch_size} (当前配置已安全)"
                )
                logger.info(
                    f"   📏 当前字符量: {estimated_chars_per_batch + prompt_overhead:,} 字符/批次"
                )

            reason = "本地模型使用简化版prompt，保持原有batch_size"
    ...
    def _build_translation_mode_config(self) -> Dict[str, str]:
        ...
        return {
            "name": str(
                getattr(self.settings.processing, "translation_mode", "Default")
            ),
    ...
    def execute(self) -> None:
        ...
        mode_name = (
            getattr(self.settings.processing.translation_mode_entity, "name", "Default")
            if self.settings.processing.translation_mode_entity
            else "Default"
        )
    ...
    def _initialize_translator(self) -> None:
        """初始化翻译器和缓存管理器"""
        provider = (
            getattr(self.settings.api, "translator_provider", "gemini") or "gemini"
        ).lower()
    ...
    def _post_translate_titles(self) -> None:
        ...
        for seg in self.all_segments:
            if (
                seg.is_new_chapter
                and seg.chapter_title
                and seg.chapter_title.strip()
                and not is_likely_chinese(seg.chapter_title)
            ):
        ...
        if self.doc_title in translation_map:
            ...
            if translated:
                ...
                logger.info(
                    f"   - 文档标题翻译: {self.doc_title} -> {self.translated_doc_title}"
                )
    ...
    def _prompt_glossary_edit(self) -> None:
        ...
        if preview_count > 0:
            ...
            for i, (term, translation) in enumerate(
                list(self.glossary.items())[:preview_count]
            ):
        ...
        try:
            user_input = (
                input("📝 编辑完成后按 Enter 继续翻译（输入 'q' 取消）: ")
                .strip()
                .lower()
            )
    ...
    def _generate_glossary(self) -> None:
        ...
        if glossary_path.exists():
            try:
                ...
                logger.info(
                    f"📚 从缓存加载已有术语表 ({len(self.glossary)} 条) -> {glossary_path}"
                )
        ...
        # 检查是否启用渐进式术语表提取
        enable_progressive = getattr(
            self.settings.processing, "enable_progressive_glossary", True
        )
    ...
    def _generate_glossary_progressive(self, pre_count: int, glossary_path) -> None:
        ...
        logger.info(
            f"   - 饱和度阈值: {self.settings.processing.glossary_stop_threshold}"
        )
        ...
        for i in range(0, len(pending_pre), batch_size):
            ...
            logger.info(
                f"🔄 处理 Batch {batch_num}/{total_batches} ({len(batch)} 个片段)..."
            )
            ...
            if batch:
                context = self._get_context_from_memory(
                    batch[0], self.settings.processing.max_context_length
                )
            ...
            for seg, t in zip(batch, batch_results):
                ...
                if (
                    self.checkpoint
                    and t
                    and not t.startswith("[Failed")
                    and not t.endswith("Failed]")
                ):
            ...
            if hasattr(self.translator, "extract_glossary"):
                try:
                    ...
                    logger.info(
                        f"   ✅ Batch {batch_num} 提取术语: {len(batch_glossary)} 条，新增: {new_terms} 条，累计: {after_count} 条"
                    )

                    # 检查是否达到最大术语数
                    if after_count >= max_terms:
                        logger.info(
                            f"✅ 术语表已达到最大条目数 ({max_terms} 条)，提前结束预翻译"
                        )
                        logger.info(
                            f"   - 已处理: {batch_num}/{total_batches} batches ({(batch_num/total_batches)*100:.1f}%)"
                        )
                        break
                    ...
                    if after_count >= min_terms:
                        ...
                        if len(new_terms_history) >= 3:
                            avg_new_terms = sum(new_terms_history) / len(
                                new_terms_history
                            )
                            ...
                            if recent_avg < avg_new_terms * stop_threshold:
                                ...
                                logger.info(
                                    f"   📊 术语增长放缓: 近3批平均 {recent_avg:.1f} < 历史平均 {avg_new_terms:.1f} × {stop_threshold}"
                                )

                                if consecutive_low_batches >= 2:  # 连续 2 次低增长
                                    logger.info(
                                        f"✅ 术语表已饱和（{after_count} 条），提前结束预翻译"
                                    )
                                    logger.info(
                                        f"   - 已处理: {batch_num}/{total_batches} batches ({(batch_num/total_batches)*100:.1f}%)"
                                    )
                                    logger.info(
                                        f"   - 节省: {total_batches - batch_num} batches"
                                    )
                                    break
                            else:
    ...
    def _generate_glossary_traditional(self, pre_count: int, glossary_path) -> None:
        ...
        if pending_pre:
            ...
            logger.info("   - 使用基础缓存（无 mode、无 glossary）")
    ...
    def _run_translation_loop(self) -> None:
        ...
        # 判断是否需要重新翻译预翻译部分
        reprocess_pretranslated = getattr(
            self.settings.processing, "reprocess_pretranslated", True
        )
        ...
        if reprocess_pretranslated:
            ...
            try:
                ratio = getattr(
                    self.settings.processing, "glossary_preamble_ratio", 0.1
                )
                pre_count = max(1, int(len(self.all_segments) * float(ratio)))
            except Exception:
                pre_count = max(1, int(len(self.all_segments) * 0.1))
        ...
        # 判断是否使用异步模式
        use_async = (
            self.settings.processing.enable_async
            and len(pending_segments) >= self.settings.processing.async_threshold
            and hasattr(self.translator, "async_translator")
        )
                ...
                try:
                    ...
                    with Progress(
                        ...
                        task = progress.add_task(
                            "[cyan]同步翻译中...", total=len(pending_segments)
                        )
                        ...
                        for i in range(0, len(pending_segments), batch_size):
                            ...
                            for seg, trans in zip(batch, results):
                                if (
                                    trans
                                    and not trans.startswith("[Failed")
                                    and not trans.endswith("Failed]")
                                ):
                                    seg.translated_text = trans
                                    self.checkpoint.mark_segment_completed(
                                        seg.segment_id
                                    )
                                    success_count += 1
                                else:
                                    seg.translated_text = (
                                        trans if trans else "[Failed: Empty response]"
                                    )
                                    self.checkpoint.mark_segment_failed(
                                        seg.segment_id, trans or "Empty response"
                                    )
                                    if isinstance(trans, str) and trans.startswith(
                                        "[Failed: Blocked"
                                    ):
                                        try:
                                            self._record_blocked_segments(
                                                [seg], reason=trans
                                            )
                                        except Exception:
                                            logger.debug(
                                                "Failed to record blocked segment"
                                            )
                            ...
                            if (
                                i // batch_size + 1
                            ) % self.settings.processing.checkpoint_interval == 0:
                                self._save_structure_map(self.all_segments)
                                self.checkpoint.save_checkpoint()

                        logger.info(
                            f"✅ 同步翻译完成: {success_count}/{len(pending_segments)} 成功"
                        )
    ...
    def _run_async_translation(self, pending_segments: SegmentList) -> None:
        ...
        # 检查translator是否支持异步
        if (
            not hasattr(self.translator, "async_translator")
            or self.translator.async_translator is None
        ):
        ...
        logger.info(
            f"   📊 共 {total_batches} 批次，批大小 {batch_size}，并发度 {max_concurrent}"
        )
        ...
        async def _process_single_batch(
            batch_idx: int, batch: SegmentList, semaphore: asyncio.Semaphore
        ):
            ...
            async with semaphore:
                try:
                    ...
                    with lock:
                        for seg, trans in zip(batch, batch_results):
                            if trans and not (
                                ...
                                and (
                                    trans.startswith("[Failed")
                                    or trans.endswith("Failed]")
                                )
                            ):
                                seg.translated_text = trans
                                self.checkpoint.mark_segment_completed(seg.segment_id)
                    ...
                    logger.info(
                        f"✅ 批次 {batch_idx}/{total_batches} 完成 (本批成功: {batch_success}/{len(batch)}, 总进度: {stats['completed_batches']}/{total_batches})"
                    )
        ...
        if RICH_AVAILABLE:
            ...
            with Progress(
                ...
                batch_task = progress.add_task(
                    f"[cyan]批次进度 (并发:{max_concurrent})", total=total_batches
                )
                segment_task = progress.add_task(
                    "[green]片段进度", total=total_segments
                )
                ...
                async def _run_with_progress():
                    ...
                    for coro in asyncio.as_completed(tasks):
                        try:
                            ...
                            # 更新进度条
                            progress.update(
                                batch_task, completed=stats["completed_batches"]
                            )
    ...
    def _cleanup_resources(self) -> None:
        ...
        try:
            if (
                hasattr(self.translator, "_async_translator")
                and self.translator._async_translator
            ):
                self.translator._async_translator.cleanup()
            if (
                hasattr(self.translator, "cache_manager")
                and self.translator.cache_manager
            ):
    ...
    def _record_blocked_segments(
        self, segments: List[ContentSegment], reason: str | None = None
    ) -> None:
        """
        ...
        try:
            ...
            for seg in segments:
                entry = {
                    ...
                    "original_text": (
                        (seg.original_text[:1000] + "...")
                        if len(seg.original_text) > 1000
                        else seg.original_text
                    ),
    ...
    def _get_context_from_memory(
        self, current_segment: ContentSegment, max_length: int
    ) -> str:
        """
    ...
    def _build_segment_index(self) -> None:
        ...
        self._segment_index = {
            seg.segment_id: idx for idx, seg in enumerate(self.all_segments)
        }
