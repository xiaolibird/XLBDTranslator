...
class SettingsBuilder:
    ...
    def _validate_settings(self) -> None:
        ...
        if (
            not self._settings.processing.enable_cache
        ) and self._settings.processing.enable_gemini_caching:
            logger.warning(
                "⚠️ enable_cache=False 时将禁用 enable_gemini_caching（避免缓存配置矛盾）"
            )
        ...
        if not self._settings.processing.enable_async:
            if (
                self._settings.processing.async_max_workers != 10
                or self._settings.processing.async_threshold != 10
            ):
                logger.warning(
                    "⚠️ enable_async=False：async_max_workers/async_threshold 仅保留配置但会被忽略"
                )

        # 3) Gemini caching 关闭时，TTL 仅提示
        if (
            not self._settings.processing.enable_gemini_caching
            and self._settings.processing.cache_ttl_hours != 1
        ):
            logger.warning(
                "⚠️ enable_gemini_caching=False：cache_ttl_hours 配置将被忽略"
            )
