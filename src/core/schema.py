...
class TranslationMode(BaseModel):
    ...
    context_len: ContextLength = Field(
        default=ContextLength.MEDIUM, description="上下文长度"
    )
...
class DocumentConfig(BaseModel):
    ...
    custom_toc_path: Optional[Path] = Field(
        None, validation_alias="CUSTOM_TOC_PATH", description="自定义目录CSV文件路径"
    )
    page_range: Optional[tuple[int, int]] = Field(
        None, description="页面范围 (start, end)"
    )
    ...
    def validate_page_range(cls, v):
        ...
        if isinstance(v, str):
            try:
                ...
                if not isinstance(parsed_list, list) or len(parsed_list) != 2:
                    raise ValueError(
                        "Page range string must be a list of 2 integers or a comma/hyphen separated string."
                    )
        ...
        if start < 0 or end < start:
            raise ValueError(
                "Invalid page range (start cannot be negative, end cannot be less than start)"
            )
        return v
...
class ContentSegment(BaseModel):
    ...
    content_type: Literal["text", "image"] = Field(
        default="text", description="内容类型"
    )
    image_path: Optional[str] = Field(
        default=None, description="图片路径 (仅当content_type为image时)"
    )
    ...
    def get_context_window(
        self, all_segments: list["ContentSegment"], window_size: int = 3
    ) -> str:
        """获取上下文窗口"""
        if not all_segments:
            return ""
        ...
        current_idx = next(
            (
                ...
                if seg.segment_id == self.segment_id
            ),
            -1,
        )
        if current_idx == -1:
...
class APISettings(BaseModel):
    ...
    translator_provider: str = Field(
        "gemini",
        validation_alias="TRANSLATOR_PROVIDER",
        description="翻译器提供商 (gemini, openai-compatible, ollama)",
    )
    gemini_api_key: Optional[str] = Field(
        None, validation_alias="GEMINI_API_KEY", description="Gemini API密钥"
    )
    ...
    openai_base_url: str = Field(
        ...
        description="OpenAI兼容API基础URL",
    )
    openai_model: str = Field(
        "gemma3", validation_alias="OPENAI_MODEL", description="OpenAI兼容模型名称"
    )
    openai_api_key: Optional[str] = Field(
        None, validation_alias="OPENAI_API_KEY", description="OpenAI兼容API密钥"
    )
    # Ollama配置
...
class FileSettings(BaseModel):
    ...
    document_path: Optional[Path] = Field(
        None, validation_alias="DOCUMENT_PATH", description="待翻译的文档路径"
    )
    output_base_dir: Path = Field(
        "output",
        validation_alias="OUTPUT_DIR",
        description="缓存和中间文件的主输出目录",
    )
    final_output_dir: Optional[Path] = Field(
        None,
        validation_alias="FINAL_OUTPUT_DIR",
        description="最终翻译文件的输出目录 (默认与源文件同目录)",
    )

    log_file: Optional[Path] = Field(
        "logs/default.log",
        validation_alias="LOG_FILE",
        description="日志文件路径 (默认不输出到文件)",
    )
    modes_config_path: Path = Field(
        Path("config/modes.json"),
        validation_alias="MODES_CONFIG_PATH",
        description="翻译模式配置文件路径",
    )
...
class ProcessingSettings(BaseModel):
    ...
    translation_mode: str = Field(
        "1", validation_alias="TRANSLATION_MODE", description="默认翻译模式ID"
    )
    batch_size: int = Field(
        5, validation_alias="BATCH_SIZE", description="批量翻译大小"
    )
    max_context_length: int = Field(
        4096, validation_alias="MAX_CONTEXT_LENGTH", description="最大上下文长度"
    )
    ...
    top_p: float = Field(
        0.95, validation_alias="TOP_P", description="核采样概率 (0.0-1.0)，控制多样性"
    )
    top_k: Optional[int] = Field(
        None,
        validation_alias="TOP_K",
        description="Top-K 采样 (1-100)，None 表示不限制",
    )
    max_output_tokens: int = Field(
        16384,
        validation_alias="MAX_OUTPUT_TOKENS",
        description="最大输出 token 数（不同模型限制: 8192-65535）",
    )

    # ========== 术语表配置 ==========
    glossary_preamble_ratio: float = Field(
        0.1,
        validation_alias="GLOSSARY_PREAMBLE_RATIO",
        description="预翻译比例 (0.05-0.3)，用于生成术语表",
    )
    glossary_min_terms: int = Field(
        10, validation_alias="GLOSSARY_MIN_TERMS", description="术语表最少条目数"
    )
    glossary_max_terms: int = Field(
        100, validation_alias="GLOSSARY_MAX_TERMS", description="术语表最多条目数"
    )
    enable_glossary_edit: bool = Field(
        False,
        validation_alias="ENABLE_GLOSSARY_EDIT",
        description="是否在生成后允许编辑术语表",
    )
    skip_pretranslate_if_glossary_exists: bool = Field(
        True,
        validation_alias="SKIP_PRETRANSLATE_IF_GLOSSARY_EXISTS",
        description="如果术语表已存在，跳过预翻译",
    )
    reprocess_pretranslated: bool = Field(
        True,
        validation_alias="REPROCESS_PRETRANSLATED",
        description="是否在正式翻译时重新处理预翻译部分",
    )
    enable_progressive_glossary: bool = Field(
        True,
        validation_alias="ENABLE_PROGRESSIVE_GLOSSARY",
        description="是否启用渐进式术语表提取（每个 batch 提取一次）",
    )
    glossary_stop_threshold: float = Field(
        0.8,
        validation_alias="GLOSSARY_STOP_THRESHOLD",
        description="术语表饱和度阈值 (0.5-1.0)，达到后停止预翻译",
    )

    # 性能与速率
    max_retries: int = Field(
        3, validation_alias="MAX_RETRIES", description="最大重试次数"
    )
    request_timeout: int = Field(
        60, validation_alias="REQUEST_TIMEOUT", description="API请求超时时间"
    )
    rate_limit_delay: float = Field(
        1.0, validation_alias="RATE_LIMIT_DELAY", description="普通文本请求间隔 (秒)"
    )
    vision_rate_limit_delay: float = Field(
        2.0,
        validation_alias="VISION_RATE_LIMIT_DELAY",
        description="Vision模式请求间隔 (秒)",
    )

    # 分块
    min_chunk_size: int = Field(
        200, validation_alias="MIN_CHUNK_SIZE", description="最小分块大小"
    )
    max_chunk_size: int = Field(
        2000, validation_alias="MAX_CHUNK_SIZE", description="最大分块大小"
    )
    ...
    translation_mode_entity: Optional[TranslationMode] = Field(
        None, description="翻译模式实体（TranslationMode 对象）"
    )
    ...
    enable_cache: bool = Field(
        True, validation_alias="ENABLE_CACHE", description="是否启用缓存"
    )
    use_breadcrumb: bool = Field(
        True,
        validation_alias="USE_BREADCRUMB",
        description="是否为章节标题生成面包屑导航",
    )
    render_page_markers: bool = Field(
        True,
        validation_alias="RENDER_PAGE_MARKERS",
        description="是否在Markdown中显示页码标记",
    )
    use_vision_mode: Optional[bool] = Field(
        None, validation_alias="USE_VISION_MODE", description="是否默认启用视觉模式"
    )
    retain_original: Optional[bool] = Field(
        None, validation_alias="RETAIN_ORIGINAL", description="是否保留原文"
    )
    # 其他可选设置
        ...
        if self.files.document_path:
            if not self.files.document_path.exists():
                raise FileNotFoundError(
                    f"Document not found: {self.files.document_path}"
                )
            if self.files.document_path.suffix.lower() not in [".pdf", ".epub"]:
                raise ValueError(
                    f"Unsupported file format: {self.files.document_path.suffix}. Only PDF and EPUB are supported."
                )
        return self
