...
## 2026-08-23 二轮对标追记：补 nDCG@10 / MRR，基线口径升级

对标 BEIR（`evaluation.py:86-101`，标准指标 NDCG/MAP/Recall/P@k）与 MTEB（检索任务
主指标 **nDCG@10**）后补的。原因是 **hit@1/@5 是阶跃指标**：名次在档内挪动它一律读作
"零变化"。本文件前两节的复盘里反复出现"@1/@5 全不变、逐 case 却有 5 处名次变动"，
那些变动是好是坏过去无法裁决——**"零回退"这个结论此前只有阶跃分辨率**。

`rag_bench.ndcg_at_k()`：二元 rel（gold 是"命中任一即算对"的集合），IDCG 按
min(len(gold), k) 条理想命中算，对齐 pytrec_eval 口径。`--json` 的 summary 增加
`ndcg10` / `mrr` 字段，`results[].top` 由 5 条放宽到 10 条（nDCG@10 需要）。

**87 case 全量基线（2026-08-23，检索实现无改动，仅新增度量）**：

| 模式 | @1 | @5 | nDCG@10 | MRR |
|---|---|---|---|---|
| hybrid（默认） | 63/87 | 75/87 | **0.8102** | 0.7906 |
| sparse | 58/87 | 66/87 | 0.7162 | 0.7132 |
| dense | 65/87 | 79/87 | **0.8252** | 0.8160 |
