...
2. ~~**`_STOP` 领域停用词复核**~~ —— **已于 2026-08-23 第二轮对标中做完并关闭，结论是维持现状**。当时的论证（"IDF 本就降权高频词，硬删是双重惩罚"）在细粒度 chunk 下不成立：补回 `data/model/method/…` 后 sparse nDCG@10 −0.0103、hybrid +0.0008（噪音级），已回滚。机制见下节 B4。
...
# 2026-08-23 第二轮对标（换品类）+ 存量向量全量验证

用户追问两条：「再找几个新的高 star RAG？」「旧记录是不是没按新程序入库、要不要更新？」

## 一、存量向量要不要更新：不用，已全量证伪

**这个疑问本身是对的，而且指向一个真盲区**：向量库 `built_at` 是 2026-08-23T07:56，
而请求体修复（`truncate:false` + `num_batch=8192`）是当天 10:22 提交的——库确实是
**旧程序**建的。更要命的是 `text_hash` 只织入 `model + text`、**不含请求参数**，
所以哪怕新旧请求体产出的向量不同，`sync_store` 也永远发现不了，会一直报"待嵌 0"。
R2 那 200 条抽样只覆盖 0.9%，不足以下结论。

**全量重嵌验证**（22592 条逐条重新 embed 后与库内向量比余弦，耗时 1088s）：

```
min_cos = 0.9999998212
余弦 < 0.9999   的条数：0
余弦 < 0.999999 的条数：0
```

最低的 10 条从 72 字符到 901 字符全是同一个数 `0.9999998212`——那是 float32 的精度
极限，与文本长度无关。**旧程序没有截断过任何一条**，22592 条向量全部有效，
不需要 `--full` 重建。

**但那个盲区要留档**：请求参数变更不在 `text_hash` 的感知范围内。这次靠人工全量验证
关掉了，下次若改 instruction prefix / pooling / 归一化方式，`sync_store` 同样不会
自觉重嵌。把参数指纹写进 meta 并在不匹配时**告警**（而非自动全量重建，那太贵）
是个候选改法，本批未做。

## 二、第二轮对标：换品类，不再找同类框架

上一轮 4 家都是通用 RAG 框架，共性假设（通用 splitter / ANN / 可配置阈值）与我们差异
大，15 项里一半判成"合理偏离"，证伪力被稀释。这轮挑**同形态**与**没碰过的维度**：

| 项目 | star | commit | 为什么选它 |
|---|---|---|---|
| neuml/txtai | ~11k | a10667a | 小规模 + SQLite + 无 ANN 精确扫，形态几乎与我们同构 |
| dorianbrown/rank_bm25 | ~1.1k | 47aa3dd | Python 事实标准 BM25，直接对标我们自研的公式 |
| beir-cellar/beir | ~2k | ef83d29 | 检索评测的标准口径 |
| embeddings-benchmark/mteb | ~2.7k | d2e311b | bge-m3 自己上榜的那个 benchmark，主指标口径 |

### B1 自研 BM25 vs txtai：逐字等价 ✅

`txtai/scoring/bm25.py` 与 `notes_search._bm25_search`：

| | txtai | 我们 |
|---|---|---|
| k1 / b | 1.2 / 0.75 | 1.2 / 0.75 |
| IDF | `log(1 + (total-freq+0.5)/(freq+0.5))` | `log((n-df+0.5)/(df+0.5) + 1)` |
| 打分 | `idf*(freq*(k1+1))/(freq + k1*((1-b)+b*len/avgdl))` | 同式 |

IDF 两式数学恒等，打分逐字相同。**这是最强的一条正面对标**：与我们同形态的方案，
BM25 实现与我们一字不差。

### B2 vs rank_bm25：我们更稳健（反超项）🟢

`rank_bm25.BM25Okapi` 用 **k1=1.5**（Okapi 原始论文值）和**无平滑** IDF
`log(N-df+0.5) - log(df+0.5)`——该式在 df > N/2 时**为负**，所以它必须加
`epsilon=0.25*average_idf` 的地板来兜。我们用的 Lucene 平滑式恒非负，不需要这个
hack；k1=1.2 与 Lucene / Elasticsearch / txtai 的工业默认一致。

### B3 分词：2+ 字符才是工业标准，我们原来的 3+ 是偏离 ✅（佐证 R1 修复）

txtai 的 alphanum 正则 `^\d*[a-z][\-.0-9:_a-z]{1,}$` 要求**至少 2 字符**（对标 Lucene
standard tokenizer），也就是说 **txtai 从一开始就不会丢 EM/MI/IV**。我们原本的
`[a-z]{3,}` 才是偏离工业标准的那个——这是对 R1 修复方向的独立外部佐证。

### B4 停用词表：我们的口径与 Lucene **方向相反**，但实验证明现状是对的 ⚠️→✅

txtai 用的是 Apache Lucene 标准英文停用词表（33 词，全是通用虚词）。与我们的 `_STOP`
（72 词）对比：

- **Lucene 有而我们无的 17 词**：`a an as at be by if in is it no of on or to will these`
  ——几乎全是两字母虚词。我们的 `[a-z]{3,}` 本来就看不到它们，R1 把两字母词全收之后，
  这批虚词只靠 IDF 压（R1 已验证可行）。
- **我们有而 Lucene 无的 56 词**里包含 `data / model(s) / method(s) / paper(s) /
  result(s) / study / studies / approach`——**领域实词**。

即 Lucene 删虚词留实词，我们留虚词删实词，**正好反过来**。而且真库 IDF 看着更不利：
被硬删的 `papers` IDF **7.35**（比真缩写 `em` 的 6.12 还高）、`method` 4.39、
`results` 4.25、`approach` 4.15 全部高于保留的核心术语 `missing`(3.76)；`data` 的
df=2027 与核心术语 `ehr` 的 df=2040 几乎相同，一个被删一个保留。

**做了实验（补回这 12 个领域词），结论是不采纳——原设计对**：

| 模式 | @1 | @5 | nDCG@10 |
|---|---|---|---|
| sparse 基线 | 58 | 66 | 0.7162 |
| sparse 补回 | **56** | 66 | **0.7059**（−0.0103，6 类里 5 类持平或变差，acronym −0.0615） |
| hybrid 基线 | 63 | 75 | 0.8102 |
| hybrid 补回 | 63 | 76 | 0.8110（**+0.0008，噪音级**） |

一边明确损失、一边零收益，证据不支持改动，已回滚。

**机制解释（比结论更值得记）**：`data` 的 df=2027/22592 ≈ 9% 看着"有区分力"，但那是
**chunk 粒度细造成的假象**——句级 highlight 占 18540 条，一篇论文的多数句子不提 data
不代表这篇与 data 无关。**细粒度 chunk 会系统性低估泛化词的覆盖面，让 IDF 虚高**。
`_STOP` 硬删领域词，正是在补偿这个偏差。所以"IDF 本就会降权高频词、硬删是双重惩罚"
这个论证（写在前两轮的遗留课题里）在细粒度 chunk 下**不成立**。

**⇒ 悬了两轮的 `_STOP` 遗留课题就此关闭：维持现状，理由已实证。**

### B5 评测口径：我们只有阶跃分辨率 ⚠️→已修

BEIR 的标准指标是 `NDCG@k / MAP@k / Recall@k / P@k`（`evaluation.py:86-101`，
k=1,3,5,10,100,1000），MTEB 检索任务的主指标是 **nDCG@10**。而我们的 `rag_bench`
只有 hit@1/@5——在 BEIR 里这叫 `top_k_accuracy`，是**补充**指标而非主指标。

问题是 **hit@k 是阶跃指标：名次在档内挪动一律读作"零变化"**。前两轮复盘里反复出现
"@1/@5 全不变、逐 case 却有 5 处名次变动"，那些变动是好是坏根本裁决不了——换句话说
**"零回退"这个结论过去只有阶跃分辨率**。

已给 `rag_bench` 补 `nDCG@10` + `MRR`（`ndcg_at_k()`，二元 rel，IDCG 按
min(len(gold), k) 算，对齐 pytrec_eval）。它上任第一件事就是裁决 B4 那个混杂信号
（sparse −2@1 / hybrid +1@5），给出了清晰答案。

**87 case 新基线（2026-08-23）**：

| 模式 | @1 | @5 | nDCG@10 | MRR |
|---|---|---|---|---|
| hybrid | 63 | 75 | **0.8102** | 0.7906 |
| sparse | 58 | 66 | 0.7162 | 0.7132 |
| dense | 65 | 79 | **0.8252** | 0.8160 |

## 这轮的净结论

- 存量 22592 条向量**全部有效**，不需要重建（全量实证，非抽样）。
- 自研 BM25 与 txtai **逐字等价**，比 rank_bm25 更稳健（无负 IDF、无 epsilon hack）。
- R1 的两字母修复方向被 Lucene/txtai 的 2+ 字符标准独立佐证。
- `_STOP` 遗留课题**关闭**：实验证伪了"该放开领域词"的假设，原设计正确，并拿到了
  机制解释（细粒度 chunk 让泛化词 IDF 虚高）。
- 补上 nDCG@10/MRR，度量本身从阶跃升级为连续——这是本轮**唯一的代码改动**。

> B5 追记（2026-08-29）：hybrid 展示顺序默认再经 bge-reranker 重排（rerank_hyde_experiment_2026-08.md）；「融合分/rerank 分与余弦不可比、门槛在融合前按余弦执行」的结论不变。
