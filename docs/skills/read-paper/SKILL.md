...
输出最末的 **「⚠️ 需要注意」块** 必看：索引里已有同文（别白读一遍几个月前已精读的）、元数据不全（来源 `pdf-llm`/`pdf-only`、零作者、或 DOI 与 arXiv 皆空——会退化成 `anon*` 键、书目缺 DOI/卷期页、`dedup_key` 退化成非 DOI 键；块里带「原因：…」行的多是 arXiv/Crossref 查询遇网络瞬断，网络恢复后**单独对那一篇**重跑 ingest 即可补齐，或加 `--title "精确标题"`——批量时 `--title` 不生效且会在这里报出来）、脚本草稿状态非 `ok`（见下方五态）、ingest 失败。
...
**看 `draft_status`**（五态，2026-09-04 起）：
- `ok`：有草稿、全部块通读成功——走下面正常协议（2–5 步，你亲读核验脚本草稿）。
- `degraded`：**有草稿，但部分块通读失败**——`draft_note` 写明失败块号。仍走正常协议，但失败块
  覆盖的页段草稿是空白，核验时必须自己从 PDF 补齐那几段（别把「草稿没写」当成「原文没有」）。
- `synth_failed`：块通读有成功、**汇总步失败**，无草稿——`draft_note` 带真实原因（LLM 调用异常
  或返回不可解析 + 长度/预算）。脚本轨缺失，改走「回退协议」。
- `api_error`：LLM 无额度/鉴权失败（402/401/403），无草稿——改走「回退协议」。
- `empty`：全部块通读失败，无草稿——改走「回退协议」。
无草稿的三态 ingest 末尾都会打印回退协议提示，且进「⚠️ 需要注意」块。

**看 `draft_note`**：`draft_status: ok` 时它通常为空；若出现「块笔记超汇总预算」，说明这篇太长
（块数 × 单块笔记长度 > 66000 字符），汇总时每块笔记都被均摊裁剪过——**被裁的部分不在草稿里**，
亲读核验时对论文后段（结果/局限/附录）不能依赖草稿，必须自己从 PDF 补齐。

**`--context "本批阅读目的"`**（可选）：分块 LLM 手里只有配置里的静态研究画像，凡不匹配的论文
「对我研究的联想」一节会一律写成「与我研究无关、不必强行建立关联」——而那可能正是你这批要读的
地基文献（2026-09-03 景观四篇中三篇如此）。批量 ingest 时用 `--context` 把「为什么读这批」告诉它，
只影响那一节。
...
⚠️ **finalize 打印「向量库已同步」之后还有一步：概念页刷新子进程（`build_topics.py`），最长 40 分钟**。
此前这一步在日志上完全静默，父进程 0% CPU、S 态干等子进程，两次被当成「僵死」kill 掉
（一次 28 分钟、一次 3 分 40 秒），而 kill 掉父进程**子进程并不会死**——它被 init 收养继续写
`topics/*.md` 近两小时，人再重跑 finalize 就与孤儿并发写同一批概念页、双份烧额度。
2026-09-04 起的行为：
- 向量同步之后会打一行 `🧵 开始概念页刷新（…）：子进程最长 2400s（约 40 分钟）… stdout 实时落 logs/topics_refresh/…`，
  再打 `🧵 已发起概念页子进程 pid=…`；结束打 `🧵 概念页子进程 pid=… 结束：退出码 …，耗时 …s`。
  **看到第一行就说明归档已完成、进程在等概念页，不是卡死**——请等它跑完，或看那个日志文件的实时进度。
- 确实要中断：直接 Ctrl+C / kill 父进程即可，子进程组会一并终止（SIGTERM/SIGINT/SIGHUP 都转发；
  `kill -9` 拦不住，那时用 `pgrep -f build_topics.py` 手动清）。中断的代价是**本轮概念页不刷新**，
  归档本身（md/sidecar/索引/向量库）已完成，无需重跑 finalize；补概念页跑
  `PYTHONPATH=. python3.12 scripts/build_topics.py --affected-by-note "<本月手动精读 md 路径>"`。
- 若「向量库已同步」都没打印、进程就长时间 0% CPU，才是真的卡在向量同步（多为 Ollama 没起来）：
  抓栈（`py-spy dump --pid <pid>` 或 `sample <pid> 5`）再 kill，随后起 Ollama 补跑一次增量同步：
```bash
...
## 回退协议（`draft_status` 为 `api_error` / `synth_failed` / `empty` —— 脚本轨缺失时）

脚本深读那一轨不可用（API 没钱、汇总步失败、或全部块通读失败），但**交叉核验不能丢**——改用**两个 subagent 对抗生成**，
