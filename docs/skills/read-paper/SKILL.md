...
- `close_reading_final`：严格 `CloseReading` JSON —— `{"from_full_text": true, "source": "manual-pdf", "sections": [{"heading": "研究问题", "sentences": [{"text": "…", "tag": null}]}, …]}`。
  ...
  其中 **实验方法** 节（2026-08-15 新增）：实验方法（从原论文整理，标准是「他人照着能复现」）：数据集/队列与划分（比例、分层、防泄漏措施）、预处理、模型配置与超参（优化器/学习率/batch/epoch/随机种子/硬件）、评估协议与指标、基线及其配置、代码与数据可得性（仓库链接、许可）。**原文未报告的项要显式写「原文未报告：X」，不许省略、更不许推测填补。**
