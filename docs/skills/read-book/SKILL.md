...
⚠️ **大小写严格**：引句在句首时原文首字母大写，抄成小写会判不通过（实测 p.7 的
"The survival outcome…"）。**不要写 `\u201c` 之类的 unicode 转义**：多转义一层会把六个
字面字符写进正文，少转义一层会变成花引号、归一化后成为直引号而劈断引句。

⚠️ **跨页引句遇到图注会失效**：回验会拼接相邻页正文并剔除页眉页脚，但图表标题在块序里
可能落到正文之后，把跨页的句子从中间隔断（实测 p.17/18 之间隔着图 1.3 的图注）。
这类句子改为只引落在同一页内的部分。

另：书上也会有印刷错误。实测 Little & Rubin 3e 的 p.14 印作
"inferences to be **the** valid without modeling"（多一个 the）——回验判不通过是对的，
应回避该句或标 [sic]，不要改原文去迁就。
