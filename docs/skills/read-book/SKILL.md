...
### 2. questions —— 按书扩展分诊轴（**常常必要**）

`config/topics.yaml` 的问题是**札记库的概念页轴**，未必覆盖这本书对论文的价值。
实测：JAMA Users' Guides 按那 8 问分诊，Harm 章得 0 分——裁决实质正确（那章确实不谈
缺失机制），但结论「整本不用读」是错的，因为那本书的价值在方法学评价框架。
